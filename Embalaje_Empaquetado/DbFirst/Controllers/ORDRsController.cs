...
namespace DbFirst.Controllers
{
    public class ORDRsController : Controller
    {
        ...
        public JsonResult GetProductos(int DocNum)
        {
            ...
            var oRDR = db.ORDRs.FirstOrDefault(p => p.DocNum == DocNum);
            List<Producto_Existencias> _ListaProductos = new List<Producto_Existencias>();
            Logic newlogic = new Logic();
            if (oRDR != null) {
                var ProductoPedido = db.RDR1.Where(p => p.DocEntry == oRDR.DocEntry);
            ...
            return Json(new { data = _ListaProductos
    }, JsonRequestBehavior.AllowGet);

        }
        public JsonResult Getpruebagrid(int cantidad, string lts)
        {
            Logic newlogic = new Logic();
            List<Producto_Existencias> _ListaProductos = new List<Producto_Existencias>();
            ...
            list = lts.Split(',').ToList();
           

            foreach (var item in list)
            {
                var ProductoPedido = db.RDR1.FirstOrDefault(p => p.ItemCode == item );
                Producto_Existencias prod = new Producto_Existencias();
                prod.DocEntry = ProductoPedido.DocEntry;
                prod.Dscription = ProductoPedido.Dscription;
                prod.ItemCode = ProductoPedido.ItemCode;
                prod.Price = ProductoPedido.Price;
                prod.TaxCode = ProductoPedido.TaxCode;
                prod.Quantity = ProductoPedido.Quantity;
                var exist = newlogic.BuscarExistencias(ProductoPedido.ItemCode, ProductoPedido.WhsCode);
                prod.Existencias = exist.OnHand;
                if (prod.Existencias < ProductoPedido.Quantity)
                {
                    prod.Active = true;
                }
                else
            ...
            string json = JsonConvert.SerializeObject(_ListaProductos);
